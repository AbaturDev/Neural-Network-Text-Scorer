...
def prepare_data():
    ...
    scaler = MinMaxScaler()
    y_commonlit = scaler.fit_transform(df_commonlit[["flesch_reading_ease"]].fillna(0)).flatten()

    X_jfleg = process_texts(df_jfleg["sentence"].astype(str))
    y_jfleg = df_jfleg["corrections"].astype(str)
    
    X_jfleg_train_raw, X_jfleg_test_raw, y_jfleg_train_raw, y_jfleg_test_raw = train_test_split(
        X_jfleg, y_jfleg, test_size=0.2, random_state=42
    )
